     ...
     public GameObject[] Spawn(int n)
     {
        ...
        //{
            ...
            for (int i = 0; i < n; i++)
            {
                if (i == 0)
                {
                    ...
                    targets[i].GetComponent<MeshRenderer>().material.mainTexture = texture3;
                }
                else if (i == 1)
                {
        ...
        if (n % 5 == 0)
        {
            ...
            */
            ...
            {
                //specialTarget = Instantiate(sampleTarget, spawnPos1, sampleTarget.transform.rotation);
                specialTarget.GetComponent<MeshRenderer>().material.mainTexture = texture1 ;
            }
            else
            {
                specialTarget.GetComponent<MeshRenderer>().material.mainTexture = texture2;
            }
            for(int i=0; i<targets.Length; i++)
            {
                if(targets[i].transform.position == specialTarget.transform.position)
                {
                    Destroy(targets[i]);
                }
            }
